...
export interface MonthlyBillingSummary {
  ...
  jobsEnqueued: unknown[];
  totalBuildings: number;
}

function createEmptySummary(billingMonth: string): MonthlyBillingSummary {
  return {
    billingMonth,
    jobsEnqueued: [],
    totalBuildings: 0,
...
export async function runMonthlyBilling(targetMonth?: string): Promise<MonthlyBillingSummary> {
  ...
  return db.transaction(async (tx): Promise<MonthlyBillingSummary> => {
    ...
    if (existingRentDebit) {
      ...
      return createEmptySummary(billingMonth);
    }
    ...
    const summary = createEmptySummary(billingMonth);
...
async function getPreviousMonthBalanceTx(
  tx: any,
  tenantId: string,
  ...
  const [result] = await tx
    ...
    .from(tenantLedger)
    .where(and(eq(tenantLedger.tenantId, tenantId), lte(tenantLedger.billingMonth, prevMonthStr)));
